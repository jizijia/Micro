...
namespace Micro.Core.Infrastructure
{
    /// <summary>
    /// 领域类型查找器
    ...
    public class AppDomainTypeFinder : ITypeFinder
    {
        ...
        /// 获取Assembly集合
        /// </summary>
        /// <param name="directoryPath">相对路径</param>
        /// <param name="searchPattern">匹配字符串</param>
        /// <param name="topDirectoryOnly">是否只有第一级目录</param>
        public IList<Assembly> GetAssemblies(string directoryPath, string searchPattern = "", bool topDirectoryOnly = true)
        {
            var binPath = _fileProvider.MapPath(directoryPath); 
            var files = _fileProvider.GetFiles(binPath, "*.dll", topDirectoryOnly).Where(x =>
            {
                string fileName = x.Substring(x.LastIndexOf(@"\")+1, x.Length - x.LastIndexOf(@"\") - 1);
                return Regex.IsMatch(fileName, searchPattern);
            }).ToList().Distinct();
            return files.Select(x => Assembly.LoadFrom(x)).ToList();
        }
